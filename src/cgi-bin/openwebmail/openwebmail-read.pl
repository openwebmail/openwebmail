...
sub readmessage {
   ...
   if (is_convertable($convfrom, $prefs{'charset'}) ) {
	...
	=iconv($convfrom, $prefs{'charset'},$from,$replyto,$to,$cc,$subject,$body);
   }
   ...
   $temphtml = iconlink("backtofolder.gif", "$lang_text{'backto'} $printfolder", qq|href="$main_url&amp;action=displayheaders"|).qq| &nbsp; \n|;
   $html =~ s/\@\@\@BACKTOLINK\@\@\@/$temphtml/g;

   if ($folder eq 'saved-drafts') {
      $temphtml .= iconlink("editdraft.gif",    $lang_text{'editdraft'},    qq|href="$send_url_with_id&amp;action=composemessage&amp;composetype=editdraft"|) . qq| \n|;
   } elsif ($folder eq 'sent-mail') {
      $temphtml .= iconlink("editdraft.gif",    $lang_text{'editdraft'},    qq|href="$send_url_with_id&amp;action=composemessage&amp;composetype=editdraft"|) . qq| \n|.
                   iconlink("forward.gif",      $lang_text{'forward'},      qq|href="$send_url_with_id&amp;action=composemessage&amp;composetype=forward&amp;convfrom=$convfrom"|) . qq| \n|.
                   iconlink("forwardasatt.gif", $lang_text{'forwardasatt'}, qq|href="$send_url_with_id&amp;action=composemessage&amp;composetype=forwardasatt"|) . qq| \n|;
   } else {
      $temphtml .= iconlink("compose.gif",      $lang_text{'composenew'},   qq|href="$send_url&amp;action=composemessage"|) . qq| \n|.
                   iconlink("reply.gif",        $lang_text{'reply'},        qq|href="$send_url_with_id&amp;action=composemessage&amp;composetype=reply&amp;convfrom=$convfrom"|) . qq| \n|.
                   iconlink("replyall.gif",     $lang_text{'replyall'},     qq|href="$send_url_with_id&amp;action=composemessage&amp;composetype=replyall&amp;convfrom=$convfrom"|) . qq| \n|.
                   iconlink("forward.gif",      $lang_text{'forward'},      qq|href="$send_url_with_id&amp;action=composemessage&amp;composetype=forward&amp;convfrom=$convfrom"|) . qq| \n|.
                   iconlink("forwardasatt.gif", $lang_text{'forwardasatt'}, qq|href="$send_url_with_id&amp;action=composemessage&amp;composetype=forwardasatt"|) . qq| \n|;
   }
   $temphtml .= "&nbsp; \n";
   $temphtml .= iconlink("print.gif", $lang_text{'printfriendly'}, qq|onclick="javascript:window.open('$read_url_with_id&amp;action=readmessage&amp;headers=$headers&amp;attmode=simple&amp;convfrom=$convfrom&amp;printfriendly=yes','PrintWindow', 'width=720,height=360,resizable=yes,menubar=yes,scrollbars=yes')"|) . qq| \n|;
   if ($folder ne 'mail-trash') {
      my $url=qq|href="$main_url&amp;action=movemessage&amp;message_ids=$escapedmessageid&amp;message_id=$escapedmessageaftermove&amp;destination=mail-trash&amp;headers=$headers&amp;attmode=$attmode|;
      ...
      $temphtml .= iconlink("totrash.gif", $lang_text{'totrash'}, $url). qq| \n|;
   }
   $temphtml .= "&nbsp; \n";
   if ($config{'enable_calendar'}) {
      $temphtml .= iconlink("calendar.gif", $lang_text{'calendar'}, qq|href="$config{'ow_cgiurl'}/openwebmail-cal.pl?action=calmonth&amp;sessionid=$thissession&amp;folder=$escapedfolder&amp;message_id=$escapedmessageid"|) . qq| \n|;
   }
#   $temphtml .= iconlink("logout.gif","$lang_text{'logout'} $prefs{'email'}", qq|href="$main_url&amp;action=logout"|) . qq| \n|;
   ...
   if (defined($message{"prev"})) {
      $temphtml1 = iconlink("left.gif", "&lt;", qq|href="$read_url&amp;action=readmessage&amp;message_id=|.escapeURL($message{'prev'}).qq|&amp;headers=$headers&amp;attmode=$attmode"|) . qq|\n|;
   } else {
      $temphtml1 = iconlink("left-grey.gif", "", ""). qq|\n|;
   }
   if (defined($message{"next"})) {
      $temphtml2 = iconlink("right.gif", "&gt;", qq|href="$read_url&amp;action=readmessage&amp;message_id=|.escapeURL($message{'next'}).qq|&amp;headers=$headers&amp;attmode=$attmode"|). qq|\n|;
   } else {
   ...
   if ( $message{'charset'} ne $prefs{'charset'} &&
      ...
      my @cflist;
      push(@cflist, 'none.msgcharset') if (lc($message{'charset'}) ne lc($prefs{'charset'}));
      ...
      if ($#cflist>0) {
         ...
         $temphtml .= hidden(-name=>'message_id',
                             ...
                             -override=>'1');
         $html =~ s/\@\@\@STARTCONVFROMFORM\@\@\@/$temphtml/g;
         ...
         $temphtml = $lang_text{'charset'}. "&nbsp;".
                     popup_menu(-name=>'convfrom',
                                ...
                                -override=>'1');
         $html =~ s/\@\@\@CONVFROMMENU\@\@\@/$temphtml/g;

         $temphtml = end_form();
         $html =~ s/\@\@\@ENDCONVFROMFORM\@\@\@/$temphtml/g;
      } else {
         $html =~ s/\@\@\@STARTCONVFROMFORM\@\@\@//g;
         $html =~ s/\@\@\@CONVFROMMENU\@\@\@//g;
         $html =~ s/\@\@\@ENDCONVFROMFORM\@\@\@//g;
      }
   } else {
      $html =~ s/\@\@\@STARTCONVFROMFORM\@\@\@//g;
      $html =~ s/\@\@\@CONVFROMMENU\@\@\@//g;
   ...
   if ( $folder ne 'saved-drafts' && $folder ne 'sent-mail' &&
      ...
      if ( -f "$folderdir/.stationery.book" ) {
         ...
         $temphtml = startform(-action=>"$config{'ow_cgiurl'}/openwebmail-send.pl",
                               -name=>'ReplyWith');
         $temphtml .= hidden(-name=>'sessionid',
                             -value=>$thissession,
                             -override=>'1');
         ...
         $temphtml .= hidden(-name=>'composetype',
                             -value=>'reply',
                             -override=>'1');
         $html =~ s/\@\@\@STARTSTATIONERYFORM\@\@\@/$temphtml/g;

         $temphtml .= popup_menu(-name=>'statname',
                                 -"values"=>\@stationery,
                                 -labels=>\%escstat,
                                 -onChange=>'JavaScript:document.ReplyWith.submit();',
                                 -override=>'1');
         $html =~ s/\@\@\@STATIONERYMENU\@\@\@/$temphtml/g;
      } else {
         $html =~ s/\@\@\@STARTSTATIONERYFORM\@\@\@/<form>/g;
         $html =~ s/\@\@\@STATIONERYMENU\@\@\@//g;
      }

      $temphtml = iconlink("editst.gif", $lang_text{'editstat'}, qq|href="$config{'ow_cgiurl'}/openwebmail-prefs.pl?action=editstat&amp;sessionid=$thissession&amp;sort=$sort&amp;folder=$escapedfolder&amp;firstmessage=$firstmessage&amp;message_id=$escapedmessageid"|). qq| \n|;
      $html =~ s/\@\@\@STATIONERYBUTTON\@\@\@/$temphtml/g;

   } else {
      $html =~ s/\@\@\@STARTSTATIONERYFORM\@\@\@/<form>/g;
      $html =~ s/\@\@\@STATIONERYMENU\@\@\@//g;
      $html =~ s/\@\@\@STATIONERYBUTTON\@\@\@//g;
   }
   ...
   if ($headers eq "all") {
      ...
      if ($printfriendly ne "yes") {
         $temphtml .= qq|&nbsp;|. iconlink("import.s.gif",   "$lang_text{'importadd'} $eaddr",  qq|href="$config{'ow_cgiurl'}/openwebmail-abook.pl?action=addaddress&amp;sessionid=$thissession&amp;sort=$sort&amp;firstmessage=$firstmessage&amp;folder=$escapedfolder&amp;message_id=$escapedmessageid&amp;realname=|.escapeURL($ename).qq|&amp;email=|.escapeURL($eaddr).qq|&amp;usernote=_reserved_" onclick="return confirm('$lang_text{importadd} $eaddr ?');"|) . qq|\n|;
         $temphtml .= qq|&nbsp;|. iconlink("blockemail.gif", "$lang_text{'blockemail'} $eaddr", qq|href="$config{'ow_cgiurl'}/openwebmail-prefs.pl?action=addfilter&amp;sessionid=$thissession&amp;sort=$sort&amp;firstmessage=$firstmessage&amp;folder=$escapedfolder&amp;message_id=$escapedmessageid&amp;priority=20&amp;rules=from&amp;include=include&amp;text=$eaddr&amp;destination=mail-trash&amp;enable=1" onclick="return confirm('$lang_text{blockemail} $eaddr ?');"|) . qq|\n|;
         if ($message{smtprelay} !~ /^\s*$/) {
            $temphtml .= qq|&nbsp; |.iconlink("blockrelay.gif", "$lang_text{'blockrelay'} $message{smtprelay}", qq|href="$config{'ow_cgiurl'}/openwebmail-prefs.pl?action=addfilter&amp;sessionid=$thissession&amp;sort=$sort&amp;firstmessage=$firstmessage&amp;folder=$escapedfolder&amp;message_id=$escapedmessageid&amp;priority=20&amp;rules=smtprelay&amp;include=include&amp;text=$message{smtprelay}&amp;destination=mail-trash&amp;enable=1" onclick="return confirm('$lang_text{blockrelay} $message{smtprelay} ?');"|) . qq|\n|;
         }
      }
      $temphtml .= "<BR>";
   ...
   if ($do_cutfolders) {
      ...
      $charset=$message{'charset'} if ($convfrom eq 'none.msgcharset');
   ...
   if ($convfrom eq 'none.msgcharset') {
      my @tmp=($prefs{'language'}, $prefs{'charset'});
      ($prefs{'language'}, $prefs{'charset'})=('en', $message{'charset'});
      printfooter((($printfriendly eq "yes")?0:2));
      ($prefs{'language'}, $prefs{'charset'})=@tmp;
   } else {
      printfooter((($printfriendly eq "yes")?0:2));
   }
