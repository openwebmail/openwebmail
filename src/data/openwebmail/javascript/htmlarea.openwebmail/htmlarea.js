...
HTMLArea.Config = function () {
	...
	this.toolbar = [ [ "fontname", "space" ],
			 ...
			 [ "horizontalrule", "createlink", "insertimage", "insertsound", "inserttable", "htmlmode", "separator" ],
			 [ "about" ]
		];
	...
	this.btnList = {
		...
		insertsound:    ["InsertSound",          "Insert Sound",       "ed_sound.gif",             false],
...
HTMLArea.prototype.generate = function () {
	...
	if (HTMLArea.is_ie) {
		// tricky! set src to local url to turn off SSL security alert on IE
	...
	function initIframe() {
		...
		html += "</head>\n";
		html += "<body dir='"+editor.config.bodyDirection+"'>\n";
...
// Called when the user clicks on "InsertSound" button
HTMLArea.prototype._insertSound = function() {
	var sel = this._getSelection();
	var range = this._createRange(sel);
	var editor = this;	// for nested functions

	this._popupDialog("insert_sound.html", function(param) {
		if (!param) {	// user must have pressed Cancel
			return false;
		}
		var doc = editor._doc;
		// create the embed, a, br element
		...
		for (var field in param) {
			...
			switch (field) {
				...
				break;
			    case "f_autoplay":
				embed.autoplay=value;
				break;
			    case "f_hidden":
				embed.hidden=value;
				break;
			    case "f_loop":
				embed.loop=parseInt(value);
				embed.playcount=parseInt(value);
				break;
			    case "f_alt":
				var alt = doc.createTextNode(value);
				var ft = doc.createElement("font");
				ft.size="-1";
				ft.appendChild(alt);
				a.appendChild(ft);
				break;
		...
		if (HTMLArea.is_ie) {
			range.pasteHTML(HTMLArea.getHTML(embed, true));
			range.pasteHTML("<br>");
			range.pasteHTML(HTMLArea.getHTML(a, true));
			range.pasteHTML("<br>");
		} else {
...
HTMLArea.prototype.getHTML = function() {
	...
	html = html.replace(/<(b|i|u|em|strike|strong)>(<br ?[\/]?>|\s)*<\1>/ig, "$2")
                   .replace(/<font[^\<\>]*>(<br ?[\/]?>|\s)*<[\/]font>/ig, "$1")
                   .replace(/<(b|i|u|em|strike|strong)>(<br ?[\/]?>|\s)*<\1>/ig, "$2")
                   .replace(/<font[^\<\>]*[\/]>/ig, "")
                   .replace(/<(b|i|u|em|strike|strong) ?[\/]>/ig, "");
        // insert new line for better readability
        return html.replace(/<(div|p|table|tbody|tr|li|ol|ul|br|hr|embed)/ig, "\n<$1")
	           .replace(/<[\/](div|p|table|tbody|tr|li|ol|ul|embed)>/ig, "</$1>\n")
	           .replace(/\n\n+/g, "\n\n")
                   .replace(/<[\/](tbody|tr|li)>\n\n+/ig, "</$1>\n");
};
